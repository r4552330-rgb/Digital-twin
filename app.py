...
with st.sidebar:
    ...
    for icon, name, has_badge in nav_items:
        label = f"{icon}  {name}  ②" if has_badge else f"{icon}  {name}"
    ...
    <div style='padding:0.3rem 0.5rem;'>
        <div style='color:#475569!important;font-size:10px;'>Capacité DC</div>
...
meteo_icon  = "☀️"         if data["irradiance"] > 600 else ("⛅"      if data["irradiance"] > 200 else "☁️")
...
<div style='background:#0f172a;padding:.9rem 2rem;display:flex;align-items:center;
    ...
    <div style='display:flex;align-items:center;gap:12px;padding-right:2rem;
        ...
        <div style='font-size:34px;line-height:1;'>{meteo_icon}</div>
        <div>
    ...
    <div style='padding:0 2rem;border-right:1px solid #1e293b;margin-right:2rem;'>
        ...
        <div style='color:#94a3b8;font-size:10px;'>{res["p_ac_kw"]/config["inverter"]["p_rated_kw"]*100:.0f}% de la capacité</div>
    </div>
    <div style='padding:0 2rem;border-right:1px solid #1e293b;margin-right:2rem;'>
    ...
    <div style='padding:0 2rem;border-right:1px solid #1e293b;margin-right:2rem;'>
        <div style='color:#64748b;font-size:10px;text-transform:uppercase;letter-spacing:.06em;'>CO₂ évité</div>
    ...
    <div style='margin-left:auto;text-align:right;'>
        ...
        <div style='margin-top:6px;'>
            <select style='background:#1e293b;color:#94a3b8;border:1px solid #334155;
                ...
                <option>Période</option><option selected>Aujourd'hui</option>
...
with col_left:
    ...
    # Dropdowns overlay simulés
    overlay_cols = st.columns([1, 3])
    with overlay_cols[0]:
        vue_mode = st.selectbox("", ["Vue libre", "Irradiance", "Température", "Production", "Pertes"],
                                label_visibility="collapsed")

    # SVG panneau PV
    n = config["array"]["n_panels"]
    panel_svg = ""
    ...
    flux_data = [
        ("🔆", "PANNEAUX PV",  f"{res['p_dc_kw']:.2f} kW", "#f59e0b"),
        ("⚡", "MPPT / DC",    f"{res['p_dc_kw']:.2f} kW", "#3b82f6"),
        ("🔄", "ONDULEUR",     f"{res['p_ac_kw']:.2f} kW", "#8b5cf6"),
        ("🏭", "RÉSEAU",       f"{res['p_ac_kw']:.2f} kW", "#22c55e"),
    ]
    fc = st.columns(4)
    for col, (icon, label, val, color) in zip(fc, flux_data):
        with col:
            st.markdown(f"""
...
with col_right:
    ...
    # Onglets période
    p_cols = st.columns(4)
    for col, period in zip(p_cols, ["Jour", "Semaine", "Mois", "Année"]):
        with col:
    ...
    for col, label, val, unit in zip(
        [m1, m2, m3],
        ["Production", "Rendement spécifique", "Performance Ratio"],
    ...
    <div style='background:#0f172a;border:1px solid #1e293b;border-radius:12px;
        ...
        <div style='color:#3b82f6;font-weight:700;font-size:12px;letter-spacing:.08em;margin-bottom:.5rem;'>
            RÉPARTITION DES PERTES (AUJOURD'HUI)</div>
    </div>""", unsafe_allow_html=True)

    losses = [
        ("#f59e0b", "Pertes d'irradiance",  6.2),
        ("#3b82f6", "Pertes de température", 4.1),
        ("#8b5cf6", "Pertes onduleur",       round((1 - config["losses"]["ac_efficiency"]) * 100, 1)),
        ("#ef4444", "Pertes DC câblage",     round(config["losses"]["dc_total"] * 100, 1)),
    ]
...
<div style='background:#0a1929;border-top:1px solid #1e293b;padding:.5rem 2rem;
    ...
    <span>Mis à jour : {now.strftime("%d/%m/%Y %H:%M:%S")} | Source : {data["source"]}</span>
</div>
""", unsafe_allow_html=True)
