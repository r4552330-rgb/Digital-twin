...
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = False  # White theme par défaut
...
UI_THEME = UI_THEMES["dark" if st.session_state.dark_mode else "light"]
...
# ─────────────────────────────────────────────────────────────────────────────
# THEME OVERRIDES: white/dark mode
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(f"""
<style>
  :root {{
    --bg-main:    {UI_THEME["bg_main"]};
    --bg-card:    {UI_THEME["bg_card"]};
    --bg-card2:   {UI_THEME["bg_card2"]};
    --border:     {UI_THEME["border"]};
    --text-main:  {UI_THEME["text_main"]};
    --text-muted: {UI_THEME["text_muted"]};
  }}
  ...
  .stApp [style*="color:#e6edf3"],
  .stApp [style*="color: #e6edf3"] {{
    color: var(--text-main) !important;
  }}

  .stApp [style*="color:#8b949e"],
  .stApp [style*="color: #8b949e"] {{
    color: var(--text-muted) !important;
  }}

  .stApp [style*="border-color:#21262d"],
  .stApp [style*="border-color: #21262d"],
  .stApp [style*="border-color:#30363d"],
  .stApp [style*="border-color: #30363d"] {{
    border-color: var(--border) !important;
  }}

  .stApp [style*="border-bottom:1px solid #21262d"],
  .stApp [style*="border-bottom: 1px solid #21262d"] {{
    border-bottom-color: var(--border) !important;
  }}

  .stApp [style*="background:linear-gradient(135deg,#0d2137,#1a3a5c)"] {{
    background: linear-gradient(135deg, var(--bg-card2), var(--bg-card)) !important;
  }}
</style>
""", unsafe_allow_html=True)
...
PLOTLY_LAYOUT = dict(
    ...
    font=dict(color=UI_THEME["text_main"], size=11, family="Inter"),
    margin=dict(l=40, r=20, t=30, b=40),
    xaxis=dict(gridcolor=UI_THEME["grid"], showgrid=True, zeroline=False),
    yaxis=dict(gridcolor=UI_THEME["grid"], showgrid=True, zeroline=False),
    legend=dict(bgcolor="rgba(0,0,0,0)", borderwidth=0, font=dict(color=UI_THEME["text_main"])),
)

def apply_layout(fig, **kwargs):
    layout = {**PLOTLY_LAYOUT, **kwargs}
    fig.update_layout(**layout)
    fig.update_xaxes(gridcolor=UI_THEME["grid"], color=UI_THEME["text_main"], zeroline=False)
    fig.update_yaxes(gridcolor=UI_THEME["grid"], color=UI_THEME["text_main"], zeroline=False)
    fig.update_annotations(font_color=UI_THEME["text_main"])
    return fig
